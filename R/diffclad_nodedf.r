...
crt_diffclad_nodedf <- function(obj, classgroup, mlfun = "lda", ldascore = 3,
  ...
  diffclad_nodedf <- diff_analysis(obj,
                                classgroup,
                                mlfun = mlfun,
                                ldascore = ldascore,
  ...
  df_diffclad_nodedf <- as.data.frame(diffclad_nodedf) %>% 
                      mutate(f = gsub("D_[0-9]__", "", f)) %>%
                      dplyr::select(f,Group, pvalue)
