...
source("R/diffclad_input_obj.r") # function for create diffclad input obj
source("R/diffclad_input_nodedf.r") # function for create diffclad input nodedf
...
required_pkgs <- c("MicrobiotaProcess", "phyloseq", "ggplot2")
check_and_load_packages(required_pkgs) # Load necessary packages
...
diffclad_obj <- crt_diffclad_obj(obj = subset_physeq,classgroup = "Group", mlfun = "lda", ldascore = 3,
                              type = "species", normalization = 1000000)

# Create the ggdiffclade input (nodedf)
diffclad_nodedf <- crt_diffclad_nodedf(subset_physeq = subset_physeq)


# Create the ggdiffclade plot

p <- ggdiffclade(obj = diffclad_nodedf,
                nodedf = diffclad_obj,
                factorName = "Group",
                layout = "circular",
                skpointsize = 0.6,
